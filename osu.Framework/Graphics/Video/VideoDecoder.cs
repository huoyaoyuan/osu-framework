...
namespace osu.Framework.Graphics.Video
{
    ...
    public unsafe class VideoDecoder : IDisposable
    {
        ...
        public void Seek(double targetTimestamp)
        {
            ...
            {
                FFmpegNative.avcodec_flush_buffers(codecContext);
                FFmpegNative.av_seek_frame(formatContext, stream->index, (long)(targetTimestamp / timeBaseInSeconds / 1000.0), FFmpegNative.AVSEEK_FLAG_BACKWARD);
        ...
        private static int readPacket(void* opaque, byte* bufferPtr, int bufferSize)
        {
            ...
            return bytesRead != 0 ? bytesRead : FFmpegNative.AVERROR_EOF;
        }
        ...
        private void prepareDecoding()
        {
            ...
            byte* contextBuffer = (byte*)FFmpegNative.av_malloc(context_buffer_size);

            ioContext = FFmpegNative.avio_alloc_context(contextBuffer, context_buffer_size, 0, (void*)handle.Handle, &readPacket, null, &streamSeekCallbacks);

            var fcPtr = FFmpegNative.avformat_alloc_context();
            formatContext = fcPtr;
            formatContext->pb = ioContext;
            formatContext->flags |= FFmpegNative.AVFMT_FLAG_GENPTS; // required for most HW decoders as they only read `pts`

            int openInputResult;

            fixed (byte* ptr = "pipe:"u8)
                openInputResult = FFmpegNative.avformat_open_input(&fcPtr, ptr, null, null);
            ...
            int findStreamInfoResult = FFmpegNative.avformat_find_stream_info(formatContext, null);
            ...
            int streamIndex = FFmpegNative.av_find_best_stream(formatContext, AVMediaType.AVMEDIA_TYPE_VIDEO, -1, -1, null, 0);
            if (streamIndex < 0)
            ...
            if (stream->duration > 0)
                ...
                Duration = formatContext->duration / (double)FFmpegNative.AV_TIME_BASE * 1000.0;
        }
            ...
            foreach (var (decoder, hwDeviceType) in GetAvailableDecoders(formatContext->iformat, codecParams.codec_id, targetHwDecoders))
            {
                ...
                if (codecContext != null)
                {
                    fixed (AVCodecContext** ptr = &codecContext)
                        FFmpegNative.avcodec_free_context(ptr);
                }

                codecContext = FFmpegNative.avcodec_alloc_context3(decoder.Pointer);
                ...
                int paramCopyResult = FFmpegNative.avcodec_parameters_to_context(codecContext, &codecParams);
                ...
                {
                    int hwDeviceCreateResult = FFmpegNative.av_hwdevice_ctx_create(&codecContext->hw_device_ctx, hwDeviceType, null, null, 0);
                ...
                int openCodecResult = FFmpegNative.avcodec_open2(codecContext, decoder.Pointer, null);
        ...
        {
            var packet = FFmpegNative.av_packet_alloc();
            var receiveFrame = FFmpegNative.av_frame_alloc();
            ...
            finally
            {
                ...
                FFmpegNative.av_frame_free(&receiveFrame);
        ...
        private void decodeNextFrame(AVPacket* packet, AVFrame* receiveFrame)
        {
            ...
            if (packet->buf == null)
                readFrameResult = FFmpegNative.av_read_frame(formatContext, packet);
            ...
            if (readFrameResult >= 0)
            {
                ...
                if (packet->stream_index == stream->index)
                {
                    ...
                    if (sendPacketResult == -FFmpegNative.EAGAIN)
                    {
                        unrefPacket = false;
                    }
                }
            ...
            else if (readFrameResult == FFmpegNative.AVERROR_EOF)
            {
                // Flush decoder.
            ...
            else if (readFrameResult == -FFmpegNative.EAGAIN)
            {
                State = DecoderState.Ready;
                Thread.Sleep(1);
            }
        ...
        private int sendPacket(AVFrame* receiveFrame, AVPacket* packet)
        {
            ...
            int sendPacketResult = FFmpegNative.avcodec_send_packet(codecContext, packet);
            ...
            if (sendPacketResult == 0 || sendPacketResult == -FFmpegNative.EAGAIN)
            {
        ...
        private void readDecodedFrames(AVFrame* receiveFrame)
        {
            while (true)
            {
                int receiveFrameResult = FFmpegNative.avcodec_receive_frame(codecContext, receiveFrame);

                if (receiveFrameResult < 0)
                {
                    if (receiveFrameResult != -FFmpegNative.EAGAIN && receiveFrameResult != FFmpegNative.AVERROR_EOF)
                    {
                ...
                long frameTimestamp = receiveFrame->best_effort_timestamp != FFmpegNative.AV_NOPTS_VALUE ? receiveFrame->best_effort_timestamp : receiveFrame->pts;
                ...
                if (((AVPixelFormat)receiveFrame->format).IsHardwarePixelFormat())
                {
                    ...
                    if (!hwTransferFrames.TryDequeue(out var hwTransferFrame))
                        hwTransferFrame = new FFmpegFrame(returnHwTransferFrame);
                    ...
                    int transferResult = FFmpegNative.av_hwframe_transfer_data(hwTransferFrame.Pointer, receiveFrame, 0);
                ...
                else
                {
                    ...
                    FFmpegNative.av_frame_move_ref(frame.Pointer, receiveFrame);
                }
        ...
        private FFmpegFrame ensureFramePixelFormat(FFmpegFrame frame, AVPixelFormat targetPixelFormat)
        {
            ...
            swsContext = FFmpegNative.sws_getCachedContext(
                swsContext,
            ...
            if (!scalerFrames.TryDequeue(out var scalerFrame))
                scalerFrame = new FFmpegFrame(returnScalerFrame);
            ...
            {
                FFmpegNative.av_frame_unref(scalerFrame.Pointer);
                ...
                int getBufferResult = FFmpegNative.av_frame_get_buffer(scalerFrame.Pointer, 0);
            ...
            int scalerResult = FFmpegNative.sws_scale(
                swsContext,
                &frame.Pointer->data.e0, frame.Pointer->linesize, 0, height,
                &scalerFrame.Pointer->data.e0, scalerFrame.Pointer->linesize);
        ...
        private void tryDisableHwDecoding(int errorCode)
        {
            ...
            if (errorCode == -FFmpegNative.ENOMEM)
            {
        ...
        private string getErrorMessage(int errorCode)
        {
            ...
            {
                strErrorCode = FFmpegNative.av_strerror(errorCode, bufPtr, buffer_size);
            }
        ...
        )
        {
            ...
            while (true)
            {
                var avCodec = FFmpegNative.av_codec_iterate(&iterator);

                if (avCodec == null) break;

                var codec = new FFmpegCodec(avCodec);
            ...
            StopDecodingAsync().ContinueWith(_ =>
            {
                if (formatContext != null && inputOpened)
                {
                    fixed (AVFormatContext** ptr = &formatContext)
                        FFmpegNative.avformat_close_input(ptr);
                }
                ...
                if (ioContext != null)
                {
                    ...
                    FFmpegNative.av_freep(&ioContext->buffer);

                    fixed (AVIOContext** ptr = &ioContext)
                        FFmpegNative.avio_context_free(ptr);
                }
                ...
                if (swsContext != null)
                    FFmpegNative.sws_freeContext(swsContext);
